<script setup>
import { ref } from 'vue';
import { getAllUser } from '../../composable/userdata';
        ...
        <div class="flex-grow px-8 py-6 flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-6">
            ...
            <div class="w-full md:w-4/5 bg-white rounded-2xl shadow-md overflow-hidden">
                <table class="w-full table-fixed">
                    ...
                    <tbody>
                        <tr v-for="(user, index) in userList" :key="user.id" class="text-gray-500 font-bold border-b last:border-0">
                            <td class="py-2">{{ index+1  }}</td>
                            <td class="py-2">{{ user.username }}</td>
                            <td class="py-2">{{ user.name }}</td>
                            <td class="py-2">{{ user.email }}</td>
                            <td class="py-2">{{ user.role }}</td>
                            <td class="py-2">{{ dateformat(user.createdOn) }}</td>
                            <td class="py-2">{{ dateformat(user.updatedOn) }}</td>
