  ...
  <ul class="inline-flex -space-x-px text-base h-10">
    <li  >
      <button class="flex items-center justify-center px-4 h-10 ml-0 leading-tight
       text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-custom-blue hover:text-white disabled:bg-gray-400 disabled:hover:text-gray-500"
    ...
    <li >
      ...
      border-gray-300 rounded-r-lg hover:bg-custom-blue hover:text-white disabled:bg-gray-400 disabled:hover:text-gray-500"
      ...
      <div class="w-full h-5/6 bg-white shadow-md rounded-2xl ">
        ...
        <div v-for="(announcement, index) in allAnnouncement" :key="index" class="w-full h-full hover:cursor-pointer">
            <div class="" @click="router.push(`/announcement/${announcement.id}`)" @mouseover="moveup(index)"  @mouseleave="movedown(index)">
            <div class="w-full  border hover:shadow-xl duration-500 " :class="index==0? 'rounded-t-2xl':'',index==4?'rounded-b-2xl':''">
                <div class="w-full flex h-[8.72rem] flex-col pl-10 pt-5 ">
              <h1 class="font-noto text-4xl">#{{  index + 1 + currentpage * pageSize }} <span> {{  announcement.announcementTitle }}</span></h1>
              <div class="flex mt-3">
                <div class="rounded-xl bg-custom-blue text-white font-noto  p-1.5"><span class=" material-symbols-outlined  text-base">list</span>{{ announcement.announcementCategory  }}</div>
                <div v-if="myMode.mode === 'close'" class="p-2">Close ON : <span class="text-red-600">{{ dateformat(announcement.closeDate)  }}</span></div>
              </div>
              <div class="w-full flex justify-end -ml-24 -mt-11 "><span class="material-symbols-outlined duration-200  rounded-full p-2.5 text-custom-blue" :id="index">navigate_next</span></div>
            </div> 
        </div>
        </div>
        </div>
